...
class TestNeo4jConfig:
    def test_invalid_config(self):
        # Missing neo4j_uri
        instance = {"neo4j_user": "test_user", "neo4j_password": "test_password"}

        with pytest.raises(Exception):
            _ = Neo4jConfig.model_validate(instance)

        # Missing neo4j_password
        instance = {"neo4j_user": "test_user", "neo4j_uri": "test_uri"}
    ...
    def test_default_config_user(self):
        instance = {"neo4j_uri": "test_uri", "neo4j_password": "test_password"}
...
class TestGraphData:
    ...
    def test_valid_single_node(self):
        instance = {
            "nodes": [
                {"labels": ["testNode"], "key": "uid", "records": [{"uid": "test"}]}
            ]
        }
    ...
    def test_valid_single_relationship(self):
            ...
            "relationships": [
                {
                    ...
                    "records": [
                        {"_from": "test1", "_to": "test2", "test_key": "test_value"}
                    ],
                }
            ],
        }
        ...
        assert gd.relationships[0].model_dump() == {
            ...
            "records": [{"_from": "test1", "_to": "test2", "test_key": "test_value"}],
        }


class TestUploadResult:

    def test_no_records_completed(self):
        from neo4j_uploader.models import UploadResult
        from datetime import datetime

        # Initialize UploadResult with no records completed
        upload_result = UploadResult(
            records_completed=0, records_total=100, started_at=datetime.now()
        )

        # Calculate the projected time to complete
        projected_time = upload_result.projected_seconds_to_complete()

        # Assert the projected time is -1
        assert projected_time == -1
